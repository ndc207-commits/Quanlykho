...
menu=st.sidebar.radio("Menu", ["Dashboard","Kho tổng","Kho cửa hàng","Thêm sản phẩm","Sửa / Xóa sản phẩm","Nhập / Xuất","Lịch sử","Xuất Excel"])
...
# ================= KHO TỔNG =================
elif menu=="Kho tổng": st.dataframe(get_stock())

# ================= KHO CỬA HÀNG =================
elif menu=="Kho cửa hàng": st.dataframe(get_store_stock())
...
# ================= SỬA / XÓA / PHỤC HỒI =================
    ...
    df=get_products()
    st.subheader("Sửa / Xóa / Phục hồi sản phẩm")
    ...
    if not deleted_df.empty:
        ...
        recover_sku=st.selectbox("Chọn sản phẩm cần phục hồi", deleted_df["display"])
        sku_recover=deleted_df[deleted_df["display"]==recover_sku]["sku"].values[0]
        if st.button("Phục hồi sản phẩm"):
            run_commit("UPDATE products SET is_active=TRUE WHERE sku=:s",{"s":sku_recover})
            st.success("Đã phục hồi sản phẩm"); refresh()
...
# ================= NHẬP / XUẤT =================
    ...
    store_name=None
    if type_tx=="Xuất":
        if role=="admin": store_name=st.selectbox("Cửa hàng",stores["name"])
        else: store_name=run_query("SELECT name FROM stores WHERE id=:id",{"id":store_user_id}).fetchone()[0]
    ...
    if st.button("Xác nhận"):
        ...
        st.success("OK"); refresh()

# ================= LỊCH SỬ =================
elif menu=="Lịch sử":
    df=pd.read_sql("SELECT h.id,h.sku,p.name AS product_name,h.type,h.quantity,h.created_at,w.name AS warehouse,h.note FROM history h LEFT JOIN products p ON h.sku=p.sku LEFT JOIN warehouses w ON h.warehouse_id=w.id ORDER BY h.created_at DESC LIMIT 200",engine)
